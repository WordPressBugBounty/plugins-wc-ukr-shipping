...
<div id="wcus-pane-parcels" class="wcus-tab-pane">
    ...
    <div class="wcus-form-group">
        <label for="wc_ukr_shipping_np_ttn_payer_default"><?php esc_html_e('Delivery payer', 'wc-ukr-shipping'); ?></label>
        <select id="wc_ukr_shipping_np_ttn_payer_default"
                name="wc_ukr_shipping[np_ttn_payer_default]"
                class="wcus-form-control">
            <option value="Sender" <?php echo $payer === 'Sender' ? 'selected' : ''; ?>><?php esc_html_e('Sender', 'wc-ukr-shipping'); ?></option>
            <option value="Recipient" <?php echo $payer === 'Recipient' ? 'selected' : ''; ?>><?php esc_html_e('Recipient', 'wc-ukr-shipping'); ?></option>
        </select>
    </div>

    <?php $paymentMethod = wc_ukr_shipping_get_option('wcus_np_payment_method_default'); ?>
    <div class="wcus-form-group">
        <label for="wcus_np_payment_method_default"><?php esc_html_e('Payment method', 'wc-ukr-shipping'); ?></label>
        <select id="wcus_np_payment_method_default"
                name="wcus[np_payment_method_default]"
                class="wcus-form-control">
            <option value="Cash" <?php echo $paymentMethod === 'Cash' ? 'selected' : ''; ?>><?php esc_html_e('Cash', 'wc-ukr-shipping'); ?></option>
            <option value="NonCash" <?php echo $paymentMethod === 'NonCash' ? 'selected' : ''; ?>><?php esc_html_e('NonCash', 'wc-ukr-shipping'); ?></option>
        </select>
    </div>
    ...
    <?php
        HtmlHelper::switcherField(
            ...
            __('Payment control', 'wc-ukr-shipping'),
